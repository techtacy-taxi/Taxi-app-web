...
class Job {
  ...
  // αποθηκεύει ο server είναι ΗΔΗ καθαρό (αρχική τιμή − προκαταβολή) — όχι η
  // αρχική συνολική τιμή. Άρα:
  //   • Απλή προκαταβολή 10% (depositPaid && !fullyPaid): ο οδηγός εισπράττει
  //     ΟΛΟΚΛΗΡΟ το `price` (είναι ήδη το υπόλοιπο).
  //   • Πλήρης πληρωμή online (fullyPaid): ο οδηγός εισπράττει 0 — όλα
  //     πληρώθηκαν ήδη στον master. Το `price` μπαίνει στο κέρδος του
  //     (driverEarning) που του χρωστάει ο master, όχι σε μετρητά.
  //   • Καμία online πληρωμή: κανονικά, εισπράττει όλο το price.
  double get remainingToCollect => fullyPaid ? 0 : price;
