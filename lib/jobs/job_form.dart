...
class _JobFormPageState extends State<JobFormPage> {
  ...
  // Κλείνει ΑΚΡΙΒΩΣ τη δική μας οθόνη (myRoute), χωρίς να πειράξει ό,τι
  // έχει ανοίξει ΑΠΟ ΠΑΝΩ της.
  //
  // Όταν στέλνεις δουλειά «σε μένα» (ή «σε όλους» με σένα μέσα), το popup
  // διεκδίκησης ανοίγει στην ΙΔΙΑ πλοήγηση, ΠΑΝΩ από τη φόρμα, λίγα
  // χιλιοστά πριν προλάβουμε να κλείσουμε:
  //   • Σκέτο Navigator.pop() → έκλεινε ΛΑΘΟΣ οθόνη (το popup) και η φόρμα
  //     έμενε κολλημένη σε φόρτωση.
  //   • popUntil(... == myRoute) → έκλεινε τη φόρμα ΑΛΛΑ σκότωνε και το
  //     popup που ήταν από πάνω (εξαφανιζόταν ακαριαία).
  // Λύση: αν είμαστε η κορυφαία οθόνη (κανονική περίπτωση, π.χ. αποστολή
  // σε άλλον οδηγό) κάνουμε κανονικό pop ώστε να επιστραφεί σωστά το
  // result (το Ημερολόγιο Google το χρειάζεται). Αν έχει ανοίξει κάτι από
  // πάνω μας, αφαιρούμε ΜΟΝΟ τη δική μας οθόνη και αφήνουμε το popup ήσυχο.
  void _closeThisRoute(Route<dynamic>? myRoute, dynamic result) {
    ...
    if (myRoute.isCurrent) {
      nav.pop(result);
    } else {
      nav.removeRoute(myRoute);
    }
  }
